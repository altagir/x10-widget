...
static struct usb_device_id DeviceTab[] =
{
...
#endif
{
	...
	debug(DBG_INFO, "freeing buffer in callback (0x%p/0x%p)\n",
                           ...
                           ssize_t bufLen)
{
	...
	if (bufLen == 0)
	{
		error("%s: write request of 0 bytes", __FUNCTION__);
		goto unlock_exit;
	}
	...
	{
		x10Dev->intrOutBusy = 0;
		error("Couldn't submit interrupt_out_urb %d", result);
		goto unlock_exit;
	}
	result = bufLen;
                            ...
                            struct x10_ha_command* cmd)
{
	...
	debug(DBG_INFO, "Transmitting %d byte command: %02X %02X %02X %02X %02X\n", cmdLen
